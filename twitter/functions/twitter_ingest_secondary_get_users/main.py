...
def twitter_ingest_secondary_get_users(message, context):
    ...
    if users:
        ...
        for user in users:
            ...
            # check if the user is in Table Storage as primary
            is_primary_user = False
            userRows = table_service.query_entities('users', filter="PartitionKey eq '" + user.id_str + "' and RowKey eq '" + user.id_str + "'")
            for row in userRows:
                if hasattr(row,'classification'):
                    if row.classification == "primary":
                        is_primary_user = True

            if is_primary_user is True:
                # if yes, delete from this queue
                ref.document(user.id_str).delete()
                logger.info(' - '.join(['PRIMARY USER ALREADY IN TABLE STORAGE', user.id_str]))
            else:
                # if not, continue to add user to Firestore
                record = {
                    "obj": {
                        "id": user.id_str,
                        "name": user.name,
                    ...
                    "last_classified": datetime.datetime.now(datetime.timezone.utc)
                }
                ...
                row.last_classified = record["last_classified"]
        ...
        for user in users:
            # update Firestore
            ref.document(user.id_str).delete()
            logger.info(' - '.join(['PROCESSED USER DELETED FROM FIRESTORE QUEUE', user.id_str]))
