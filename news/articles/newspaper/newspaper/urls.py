...
from .custom import parse_proxy_url, urljoin
...
def redirect_back(url, source_domain):
    ...
    """
    url_pieces = parse_proxy_url(url)
    parse_data = urlparse(url_pieces[3])
...
def get_domain(abs_url, **kwargs):
    ...
    if abs_url is None:
        return None
    return parse_proxy_url(abs_url, **kwargs)[0]
...
def get_path(abs_url, **kwargs):
    ...
    if abs_url is None:
        return None
    return urlparse(parse_proxy_url(abs_url)[3], **kwargs).path
